...
public class CasClient {
	...
	public String login (String serviceUrl, String username, String password) throws CasAuthenticationException, CasProtocolException
	{
		...
		if (lt == null)
		{
			if(MyDebug.DEBUG) Log.d (TAG, "Cannot retrieve LT from CAS. Aborting authentication for '" + username + "'");
		...
		else
		{
			...
			if(MyDebug.DEBUG) Log.d(TAG,"POST " + casBaseURL + CAS_LOGIN_URL_PART + "?service=" + serviceUrl);
			...
			try
			{
				...
				if(MyDebug.DEBUG) Log.d(TAG, "User: "+username+" Pass:"+password);
				...
				for(Cookie c: cookies) {
					if(MyDebug.DEBUG) Log.d(TAG, "Cookie: " + c);
				}
				...
				if(MyDebug.DEBUG) Log.d (TAG, "POST RESPONSE STATUS=" + response.getStatusLine().getStatusCode() + " : " + response.getStatusLine().toString());
				...
				if(!validLogin) {
					if(MyDebug.DEBUG) Log.d(TAG, "Cas did not return a TGC. Wrong username/password combination?");
				...
				if (entity != null) {
					...
					if (len != -1 && len < 2048) {
						if(MyDebug.DEBUG) Log.d(TAG, EntityUtils.toString(entity));
					} else {
                       showStream(entity.getContent());
					}
				} else {
					if(MyDebug.DEBUG) Log.d(TAG, "entity is null!");
				...
				if(MyDebug.DEBUG) Log.d(TAG, "Array Headers: " + Arrays.toString(headers) );
			...
			{
				if(MyDebug.DEBUG) Log.d (TAG, "IOException trying to login : " + e.getMessage());
	...
	private TokenContainer getLTFromLoginForm (String serviceUrl, HttpContext context)
	{
		...
		try
		{
			...
			if(MyDebug.DEBUG) Log.d (TAG, "Ready to get our LT from " + casBaseURL + CAS_LOGIN_URL_PART + "?service=" + serviceUrl);
			HttpResponse response = httpClient.execute(httpGet, context);

			if (response == null){
				if(MyDebug.DEBUG) Log.d(TAG, "Egal");
			}
			if(MyDebug.DEBUG) Log.d (TAG, "Response = " + response.getStatusLine().toString());
			if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK)
			{
				if(MyDebug.DEBUG) Log.d(TAG,"Could not obtain LT token from CAS: " + response.getStatusLine().getStatusCode() + " / " + response.getStatusLine());
			}
			else
			{
				HttpEntity entity = response.getEntity();
				...
				entity.consumeContent();
				if(MyDebug.DEBUG) Log.d (TAG, "LT=" + tc.lt);
				if(MyDebug.DEBUG) Log.d (TAG, "EXECUTION=" + tc.execution);
			}
		}
		catch (ClientProtocolException e)
		{
			if(MyDebug.DEBUG) Log.d(TAG, "Getting LT client protocol exception", e);
		}
		catch (IOException e)
		{
			if(MyDebug.DEBUG) Log.d(TAG, "Getting LT io exception",e);
		}
		return tc;
	}
