...
- (void)viewDidLoad {
  ...
  self.detailType.text =
      [NSString stringWithFormat:@"Typ: %@", self.detailModal[7]];
  ...
  if ([AppData sendPIN:self.detailModal[3] pin:@"123456"]) {
    ...
    if ([active isEqualToString:@"1"]) {
      ...
      self.detailActive.text =
          [NSString stringWithFormat:@"Kurs aktiv von %@ Uhr bis %@ Uhr.",
                                     self.detailModal[4], self.detailModal[5]];
      self.detailPIN.hidden = YES;
      self.detailPinEntry.hidden = YES;
      self.detailSend.hidden = YES;
    }
...
- (void)pinHandover {
  if ([AppData sendPIN:self.detailModal[3] pin:self.detailPinEntry.text]) {
    ...
    UIAlertController *alertController2 = [UIAlertController
        alertControllerWithTitle:@"Erfolg"
                         message:
                             @"Sie sind erfolgreich in den Kurs eingetragen."
    ...
    UIAlertController *alertController = [UIAlertController
        alertControllerWithTitle:@"Fehltage"
                         message:[NSString
                                     stringWithFormat:
                                         @"Sie haben: %@ Fehltag(e)",
                                         [NSString
    ...
    UIAlertController *alertController = [UIAlertController
        alertControllerWithTitle:@"Fehler"
                         message:
                             @"Die PIN ist falsch oder der Termin abeglaufen."
                  preferredStyle:UIAlertControllerStyleAlert];
    [alertController
        addAction:[UIAlertAction actionWithTitle:@"OK"
                                           style:UIAlertActionStyleDefault
