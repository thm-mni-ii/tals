...
class mod_wstals_external extends external_api {
    ...
    public static function get_todays_appointments($userid, $courseid) {
        ...
        $params = self::validate_parameters(self::get_todays_appointments_parameters(),
    ...
    public static function insert_attendance($userid, $appointmentid, $pinum) {
        ...
        $params = self::validate_parameters(self::insert_attendance_parameters(),
            ['userid' => $userid, 'appointmentid' => $appointmentid, 'pinum' => $pinum]);

        $userid = $params['userid'];

        /*
         * This approach is used, because a simple $DB->get_record('tals_type_attendance',
         * array('description' => get_string('Present_full', 'tals')), 'id') throws error.
         * The given hint to fix this is using sql_compare_text(), but this function is not documented.
         * Therefore it can't be used (I tried, really).
         */
    ...
    public static function check_for_enabled_pin($userid, $appointmentid) {
        ...
        $params = self::validate_parameters(self::check_for_enabled_pin_parameters(),
        ...
        $appointment = $DB->get_record('tals_appointment', ['id' => $appointmentid]);
        ...
        return [
            'pin enabled' => $result->pin,
            'days absent' => $result->daysabsent,
            'already attending' => $result->alreadyattending
        ];
    }
    ...
    public static function get_days_absent($userid, $courseid) {
        $params = self::validate_parameters(self::get_days_absent_parameters(), ['userid' => $userid, 'courseid' => $courseid]);
        ...
        return ['days absent' => $status->absent];
    }
